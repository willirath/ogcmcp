...
```
✓ `quickstart.build` does NOT contain `--allow-run-as-root`
✓ `quickstart.build` contains `-mpi` in the genmake2 invocation
✓ References `Dockerfile.amd64` and `Dockerfile.arm64` (not arch-detection via uname)
