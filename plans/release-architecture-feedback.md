...
Feedback sections are marked `> **USER:**` — fill in inline.
...
MIT licence is pretty easy to bundle? And we could still allow for pulling a git release? For actually building the model and running stuff, this would be way to go anyway. But default should be to not include. Check size of souce again.
...
Check db sizes. Also see if compressible. Far future: Zenodo. But limited bandwidth. So I tend to include in release for now.
...
Let's stick to OLLAMA for now and for the academic PoV. Swapping out the embedder is an easy to separate task. Defer.
...
Let's not ship the index building for.
...
OLLAMA used in doc queries like for code?

Don't care for partial operations. If no index, then broken.
...
Let's put the academic angle into a backlog item which we defer to far later. Whether we publish
depends on whether it performs :D 
