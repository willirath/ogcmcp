...
Input files are gitignored. They are either copied from the MITgcm
verification directory (tutorial experiments) or generated from Python scripts
(custom experiments).
...
input files rather than copying them from the MITgcm submodule.
...
- **Input files are not committed.** The files in `experiments/*/input/` that
  are not text namelists are gitignored and must be provided via
